...
def predict(model, loader, device, nii_path, out_path, out_bias_path, est_bias_path, filter_type):
    model.eval()
    total_loss = 0
    outputs = []
    # starts = []
    ...
    with torch.no_grad():
        with tqdm(total=len(loader)) as pbar:
            ...
            if filter_type == 'nosmooth':
                final_field = final_field
            if filter_type == 'guass':
            ...
            if filter_type == 'bspline':
                ...
                interp_mmap = np.memmap(interp_mmap_path, dtype=float, shape=(final_field.shape[0],final_field.shape[1],final_field.shape[2]), mode='w+')
                ...
                final_field = final_field.astype(np.float32)
